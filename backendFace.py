...
def send_whatsapp_message(phone_number, message, image_paths):
    # kit.sendwhats_image(phone_number, audio_path) #implementacion de envio de audio por WhatsApp
...
# modelo tracking de desconocidos y reconocimiento facial
def run3(tipoVideo):
    cap = cv2.VideoCapture(tipoVideo)
...
def opencv_processing(tipoVideo):
    # Uso del globlal para la variable de deteccion de personas desconocidas
...
def VerificacionSesion():
    ...
    DeepFace.stream(db_path="database") #libreria para el reococimiento de emociones y rostro con la libereria deepFace
    while True:
        if cv2.waitKey(1) & 0xFF == ord("q"):
            break
